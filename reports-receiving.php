...
<div class="container-fluid">
  <div class='row'>
	...
	if($reports=='1'){
		echo "<center><h3>Purchases Cost in <br>".date("F d, Y",strtotime($f))." - ".date("F d, Y",strtotime($t))."</h3></center>";
	?>
	<div class='table-responsive'>
		...
		$query = "SELECT * FROM tbl_orders_receiving WHERE date_received BETWEEN '".strtotime($f)."' AND '".strtotime($t)."' AND deleted='0' AND type='purchase'";
		...
		if(mysql_num_rows($salesquery)!=0){
			while($row=mysql_fetch_assoc($salesquery)){
				...
				$comment_query = mysql_query("SELECT * FROM tbl_orders_receiving WHERE type='purchase' AND orderID='$orderID'");
		...
		<tfoot>
			<?php
			if(mysql_num_rows($salesquery)!=0){
				$total_purchases = mysql_query("SELECT SUM(total_cost) as total_purchases FROM tbl_orders_receiving WHERE type='purchase' AND date_received BETWEEN '".strtotime($f)."' AND '".strtotime($t)."'");
				$total_purchases=mysql_fetch_assoc($total_purchases);
				$total_freight = mysql_fetch_assoc(mysql_query("SELECT SUM(freight) as total_freight FROM tbl_orders_receiving WHERE type='purchase' AND date_received BETWEEN '".strtotime($f)."' AND '".strtotime($t)."'"));
				echo "
				<tr>
