...
if(isset($salesmanID)){
	...
	mysql_query("UPDATE tbl_ts_cart SET salesmanID='$salesmanID' WHERE accountID='$accountID'");
	header("location:sales-ts");
}elseif(isset($salesman)){
	mysql_query("UPDATE tbl_ts_cart SET salesmanID='0' WHERE accountID='$accountID'");
	header("location:sales-ts");
}else{
	header("location:sales-ts");
}
