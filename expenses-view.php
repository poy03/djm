			...
			if(isset($orderID)){
				...
				if(mysql_num_rows($order_query)!=0){
					while($order_row=mysql_fetch_assoc($order_query)){
						$date_expended = $order_row["date_expended"];
						$time_expended = $order_row["time_expended"];
					...
					Date Time: <b>$date_expended $time_expended</b><br>
					...
					<tbody>
					";
					$expenses_query = mysql_query("SELECT * FROM tbl_expenses WHERE orderID='$orderID'");
