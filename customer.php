...
<div class="container-fluid">
  <div class='row'>
	...
	if(isset($_POST["edit"])){
		...
		$selected_customer = $_POST["select"];
		$_SESSION["selectcustomer"]=$selected_customer;
	...
	<div class='col-md-2'>
	<?php if($customers_modify=='1'||$customers_add=='1'){
		echo "<label>Controls:</label>";
	}?>
	<?php if($customers_add=='1'){
		echo "<span class='btn btn-primary btn-block' name='add' id='add'><span class='glyphicon glyphicon-user'></span> Add Customer</span>";
	}?>	
	 ...
	 if(mysql_num_rows($itemquery)!=0){
		$i =0;
		 ...
		 while($itemrow=mysql_fetch_assoc($itemquery)){
			 ...
			 $tin_id = $itemrow["tin_id"];
			 $email = $itemrow["email"];
			 $credit_limit = $itemrow["credit_limit"];
			 $phone = $itemrow["phone"];
			 ...
			 <tr class='selected'>
			  ...
			  <td><a href='customer-sales?id=$customerID'>$companyname</a></td>
			  <td>$address</td>
			  <td>$email</td>
			  <td>$phone</td>
			  <td>$contactperson</td>
