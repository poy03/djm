	...
	foreach ($expense_account_array as $expense_account) {
		if($expense_account!==""&&$description[$i]!==""){
			...
			$date[$i] = strtotime($date[$i]);
				mysql_query("INSERT INTO tbl_expenses (category,expense_account,description,amount,accountID,date,terms,payee,fully_paid,date_due,date_payment,comments) VALUES ('admin','$expense_account','$description[$i]','$amount[$i]','$accountID','$date[$i]','$terms[$i]','$payee[$i]','$fully_paid','$date_due','$date_payment','$comments[$i]')");
			}
		$i++;
	}
}
