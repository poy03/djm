  ...
  <div class='row'>
	...
	if($logged==1||$logged==2){
		...
		if($expenses=='1'){
			...
			<div class="row">
			<div class="col-md-2">
				<button class="btn btn-primary btn-block" type="submit" form="expenses-accounts-form"><span class="glyphicon glyphicon-floppy-disk"></span> Save</button>
			</div>
			<div class="col-md-5">
					<form action="expenses-accounts-form" id="expenses-accounts-form" method="post" class="form-horizontal">

						<div class="form-group">
						  <label class="col-md-4">Type of Expense:</label>
						  <div class="col-md-8">
						    <select class="form-control" name="type" id="type_expense">
						    	<option value="selling" <?php echo (isset($_GET['expense'])&&$_GET['expense']=="selling"?"selected":""); ?> >Selling Expense</option>
						    	<option value="admin" <?php echo (isset($_GET['expense'])&&$_GET['expense']=="admin"?"selected":""); ?>>Admin Expense</option>
						    	<option value="capital" <?php echo (isset($_GET['expense'])&&$_GET['expense']=="capital"?"selected":""); ?>>Capital Expense</option>
						    </select>
						  </div>
						</div>

						<div class="form-group">
						  <label class="col-md-4">Expense Account:</label>
						  <div class="col-md-8">
						    <input type="text" class="form-control" name="expense_account" placeholder="Enter Expense Account">
						  </div>
						</div>
						...
						<div class="form-group">
						  <label class="col-md-4">Basic Amount:</label>
						  <div class="col-md-8">
						    <input type="number" step="0.01" min="0" class="form-control" name="amount" placeholder="Basic Amount">
						  </div>
						</div>
					</form>
			</div>
				...
				<div class="col-md-10">
					<div class="table-responsive">
						<table class="table table-hover">
							<thead>
								<tr>
									<th style="text-align: center;">Type of Expense</th>
									<th style="text-align: center;">Expense Account</th>
						...
						<tbody>
							<?php
							$query = "SELECT * FROM tbl_expenses_account";
							if(isset($_GET['expense'])){
								$query .= " WHERE type = '".$_GET['expense']."'";
							}
							$expenses_account_query = mysql_query($query);
							if(mysql_num_rows($expenses_account_query)!=0){
								while($expenses_account_row=mysql_fetch_assoc($expenses_account_query)){
									echo '
									<tr>
										<td>'.ucwords($expenses_account_row["type"]." expenses").'</td>
										<td>'.$expenses_account_row["expense_account"].'</td>
										<td>'.$expenses_account_row["description"].'</td>
										<td align="right">'.number_format($expenses_account_row["amount"],2).'</td>
										<td><a href="#" class="delete" id="'.$expenses_account_row["id"].'">&times;</a></td>
									</tr>
									';
								}
							}
